...
import type { AddOn, Hotel, Quote, RoomOffer } from '@/lib/domain/schemas';
import { buildQuery } from '@/lib/application/search-params';
import type { StayCriteria } from '@/lib/domain/schemas';
import { useLocale, useT } from '@/lib/i18n/context';
import { lBed, lFloor, lView } from '@/lib/i18n/format';
...
export function RoomDetailView({ offer, hotel, addOns, quote, criteria }: RoomDetailViewProps) {
  ...
  const facts = [
    ...
    { icon: UsersThree, label: t('room.sleepsUpTo', { n: String(room.capacity) }), tone: factTone.capacity },
          ...
          <div className="min-w-0">
            ...
            {facilities.length > 0 ? (
              <section aria-labelledby="facilities-heading" className="mt-20">
                ...
                <ul aria-label={t('room.facilitiesAria')} className="mt-5 flex flex-wrap gap-2">
            ...
            <section aria-labelledby="rate-heading" className="mt-20">
              ...
              <p className="mt-2 text-sm text-muted-foreground">{t('room.includedAtNoExtraCost')}</p>
            ...
            <section aria-labelledby="policies-heading" className="mt-20">
              ...
              <div className="mt-5 grid grid-cols-2 gap-3 sm:max-w-md">
                <div className={cn('rounded-[14px] p-4', tintSurface.stone)}>
                  ...
                  <p className="text-display mt-1 text-xl">{t('room.checkInFrom')}</p>
                </div>
                <div className={cn('rounded-[14px] p-4', tintSurface.stone)}>
                  <p className="flex items-center gap-1.5 text-xs text-muted-foreground">
                  ...
                  <p className="text-display mt-1 text-xl">{t('room.checkOutBy')}</p>
                </div>
              </div>
              {/* Only a promise the booking engine can keep: withdrawing the
                  ...
                  offering it. */}
              {lateCheckOut ? (
                <p className="mt-3 text-sm text-muted-foreground">{t('room.lateCheckOutHint')}</p>
              ) : null}
              ...
              <div className="mt-5 flex items-start gap-3 rounded-3xl bg-success/10 p-4 text-base">
                ...
                />
                <p>
                  <span className="font-medium">{ratePlan.cancellationPolicy}</span>{' '}
                  {t('room.cancellationExtra')}
                </p>
              </div>
            </section>
            ...
            <section aria-labelledby="addons-heading" className="mt-20">
              ...
              <p className="mt-2 mb-5 text-sm text-muted-foreground">{t('room.addServicesHint')}</p>
            ...
            {dining.some((addOn) => addOn.enabled && !addOn.parentId) ? (
              <section aria-labelledby="dining-heading" className="mt-20">
                <h2 id="dining-heading" className="text-display text-2xl sm:text-3xl">
                  {t('room.orderFromKitchen')}
                </h2>
                <p className="mt-2 mb-5 text-sm text-muted-foreground">{t('room.orderFromKitchenHint')}</p>
